...
### Currying function
 ...
 Currying is a technique in functional programming where a function that takes multiple arguments is transformed into a series of functions that each take a single argument. It is one of high order function but more strictly. 
...
Closures are everywhere in JS. A closure is when a function has access to its lexical scope even when it is called outside of it. Closures are created every time a function is created, at function creation time. 
   `当一个函数能够记住并访问其自己的词法作用域，即使它在词法作用域之外执行，我们就说这个函数是一个闭包。`
...
### Scope Chain
 - `Scope chain`是一个引用的链表，它定义了变量的查找顺序。当访问一个变量时，JavaScript引擎会从最近的作用域开始查找，然后沿着scope chain一直查找到全局作用域，或者直到找到为止。
 - **作用域链的构建:** 当一个函数被创建时，它会保存一个引用到其父作用域的scope chain。当这个函数被调用时，它会创建一个新的作用域，用于存放它自己的局部变量。这个新的作用域会被添加到原先保存的scope chain的前面，形成一个新的、更新的scope chain。
 - **变量解析:** 当你在函数中尝试访问一个变量时，JavaScript引擎首先会检查当前函数的局部作用域是否有这个变量。如果没有，它会沿着scope chain继续向上查找，直到找到这个变量或达到全局作用域。如果在全局作用域中仍然找不到该变量，则会引发一个引用错误。
...
- 根据词法作用域，scope 的值取上一级函数的值
`loxical scope 词法作用域: 作用域会按照写的程序而决定`
